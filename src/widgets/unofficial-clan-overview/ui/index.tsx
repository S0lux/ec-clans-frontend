...
export const UnOfficialClanDetailsWidget = ({ clanId }: { clanId: string }) => {
  const { data, isLoading, error } = useQuery(
    ClansQueries.getUnofficialClanQuery(clanId),
  );
  ...
  return (
    <div className="relative mb-10 flex h-fit flex-row gap-5 rounded-md bg-neutral-900 p-3 md:mx-12 lg:mx-24 xl:mx-36 2xl:mx-64">
      ...
      {data && (
        <Image
          src={data.iconUrl || "https://placehold.co/400/png?text=No+Logo"}
          alt={data.name}
          width={128}
          height={128}
